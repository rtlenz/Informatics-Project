(function () {
	...
	myApp.controller("myControl", function($scope, $http,$window) { //Scope defines variables that can be accessed through HTML//
		
		//data on songs
		$http.get("getmylists.php")
			...
			.then(function(response){
				if(response.status == 200){
					if(response.data.status == 'error'){
						...
						//successful
						$window.location.href ="addAttribute.html";
					}
				} else{
					alert('unexpected error');
				}
